...
class MapPage: UIViewController, CLLocationManagerDelegate, UISearchBarDelegate, MKMapViewDelegate {
    ...
    func updateWeatherUI(with weatherData: WeatherData) {
        DispatchQueue.main.async {
            ...
            // SF Symbol for Humidity
            let humidityIcon = UIImage(systemName: "humidity.fill") // SF Symbol for Humidity
            let humidityAttachment = NSTextAttachment()
            humidityAttachment.image = humidityIcon
            humidityAttachment.bounds = CGRect(x: 0, y: -2, width: 18, height: 18) // Adjust icon size
            ...
            // SF Symbol for Wind
            let windIcon = UIImage(systemName: "wind") // SF Symbol for Wind
            let windAttachment = NSTextAttachment()
            windAttachment.image = windIcon
            windAttachment.bounds = CGRect(x: 0, y: -2, width: 18, height: 18) // Adjust icon size
                ...
                DispatchQueue.main.async {
                    ...
                    if let sheet = bottomSheetVC.sheetPresentationController {
                        let customDetent = UISheetPresentationController.Detent.custom { context in
                            return 300 // अपनी मनचाही हाइट
                        }
                        
                        sheet.detents = [customDetent, .large()] // Medium की जगह Custom Height
                        sheet.prefersGrabberVisible = false // Grabber (छोटा rectangle) हटाने के लिए
                    }
