...
class MapPage: UIViewController, CLLocationManagerDelegate, UISearchBarDelegate, MKMapViewDelegate, MKLocalSearchCompleterDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    private var currentBottomSheetPosition: CGFloat = -135
    ...
    private func setupGyroscope() {
        ...
        print("Gyroscope updates started.")
    }
        ...
        CitySearchHelper.searchForCity(city: cityName, mapView: mapView, locationManager: locationManager) { [weak self] (weatherData, error) in
            ...
            if let weatherData = weatherData {
                DispatchQueue.main.async {
                    ...
                    if isFirstEverSearch {
                        let alert = UIAlertController(
                            title: "Explore More",
                            message: "Pull up the bottom sheet to view detailed weather info. Long-press on the map route to check weather at specific locations.",
                        ...
                        gotItAction.setValue(UIColor(hex: "40CBD8"), forKey: "titleTextColor")
                        ...
                        let attributedMessage = NSAttributedString(
                            string: "Pull up the bottom sheet to view detailed weather info. Long-press on the map route to check weather at specific locations.",
                            attributes: [
                        ...
                        let backgroundView = alert.view.subviews.first?.subviews.first?.subviews.first
                        backgroundView?.backgroundColor = UIColor(hex: "#333333")
                        backgroundView?.layer.cornerRadius = 12
                        
                        self.present(alert, animated: true, completion: nil)
                        UserDefaults.standard.set(true, forKey: "hasPerformedSearchBefore")
                    }
                }
            }
        }
    ...
    func completerDidUpdateResults(_ completer: MKLocalSearchCompleter) {
        ...
        print("Updated results: \(searchResults.count)")
    ...
    private func setupOtherButton() {
        ...
        NSLayoutConstraint.activate([
            ...
            otherButton.widthAnchor.constraint(equalToConstant: 60),
            otherButton.heightAnchor.constraint(equalToConstant: 60)
        ])
    }
    ...
    private func setupAlertButton() {
        ...
        NSLayoutConstraint.activate([
            ...
            alertButton.widthAnchor.constraint(equalToConstant: 60),
    ...
    private func setupSuggestionTableView() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(handleSuggestionBoxTap))
        suggestionTableView.addGestureRecognizer(tapGesture)
        tapGesture.cancelsTouchesInView = false
    }
    
    @objc private func handleSuggestionBoxTap(_ gesture: UITapGestureRecognizer) {
        let location = gesture.location(in: suggestionTableView)
        if suggestionTableView.indexPathForRow(at: location) == nil {
    ...
    private func addContentToBottomSheet() {
        ...
        if let closeCircleView = bottomSheetView.subviews.first(where: { $0.backgroundColor == UIColor(white: 1, alpha: 0.2) && $0.layer.cornerRadius == 15 }),
           let closeButton = closeCircleView.subviews.first as? UIButton,
           let expandCircleView = bottomSheetView.subviews.last(where: { $0.backgroundColor == UIColor(white: 1, alpha: 0.2) && $0.layer.cornerRadius == 15 }),
           let expandButton = expandCircleView.subviews.first as? UIButton {
            closeButton.tintColor = .gray
            closeCircleView.backgroundColor = UIColor(white: 1, alpha: 0.1)
            expandButton.tintColor = .white
            expandCircleView.backgroundColor = UIColor(white: 1, alpha: 0.2)
        }
    }
    ...
    @objc private func closeBottomSheet() {
        ...
        updateButtonColorsForState(.collapsed)
    ...
    @objc private func expandBottomSheet() {
        ...
        updateButtonColorsForState(.expanded)
    ...
    @objc private func handlePanGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        if recognizer.state == .ended {
            ...
            if currentPosition > (bottomSheetExpandedHeight + bottomSheetMediumHeight) / 2 {
                ...
                updateButtonColorsForState(.medium)
            } else {
    ...
    private func updateButtonColorsForState(_ state: BottomSheetState) {
        guard let closeCircleView = bottomSheetView.subviews.first(where: { $0.layer.cornerRadius == 15 && $0.subviews.first is UIButton && ($0.subviews.first as? UIButton)?.image(for: .normal) == UIImage(systemName: "xmark") }),
              let closeButton = closeCircleView.subviews.first as? UIButton,
              let expandCircleView = bottomSheetView.subviews.first(where: { $0.layer.cornerRadius == 15 && $0.subviews.first is UIButton && ($0.subviews.first as? UIButton)?.image(for: .normal) == UIImage(systemName: "arrow.up.left.and.arrow.down.right") }),
              let expandButton = expandCircleView.subviews.first as? UIButton else {
            return
        }
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        if !isInitialLocationSet {
            ...
            updateWeatherAndLocation(for: location)
            return
        }
        if !isShowingSearchedLocation {
            if let lastLocation = lastUpdatedLocation {
                let distance = location.distance(from: lastLocation) / 1000
                if distance >= 1.0 {
                    updateWeatherAndLocation(for: location)
                    lastUpdatedLocation = location
                }
    ...
    private func updateWeatherAndLocation(for location: CLLocation) {
        let coordinate = location.coordinate
        let geocoder = CLGeocoder()
        ...
        geocoder.reverseGeocodeLocation(location) { [weak self] (placemarks, error) in
            ...
            if let placemark = placemarks?.first {
                ...
                self.fetchWeather(for: coordinate) { weatherData in
                    if let weatherData = weatherData {
                        self.updateWeatherUI(with: weatherData)
                    }
                }
    ...
    private func addWeatherAnnotationsAlongRoute(route: MKRoute) {
        ...
        let intervalDistance: CLLocationDistance = 30
        ...
        for i in 1..<pointCount {
            ...
            accumulatedDistance += currentLocation.distance(from: lastLocation) / 1000
