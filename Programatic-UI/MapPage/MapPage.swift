...
class MapPage: UIViewController, CLLocationManagerDelegate, UISearchBarDelegate, MKMapViewDelegate, MKLocalSearchCompleterDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    @objc func handleLongPress(_ gestureRecognizer: UILongPressGestureRecognizer) {
            if gestureRecognizer.state == .ended {
                ...
                // Remove any existing weather view
                weatherInfoView?.removeFromSuperview()
                
                // Create and add the centered weather view
                weatherInfoView = createWeatherInfoView()
                if let weatherInfoView = weatherInfoView {
                    view.addSubview(weatherInfoView)
                }
                
                // Fetch weather data for the tapped coordinate
                fetchWeather(for: coordinate) { [weak self] weatherData in
                    // No additional UI updates here; handled in fetchWeather
                }
            }
        }
        
        // Create a centered weather info view with professional design
        private func createWeatherInfoView() -> UIView {
            ...
            // Default gradient background (updated later based on weather)
            let gradientLayer = CAGradientLayer()
            gradientLayer.frame = weatherView.bounds
            ...
            weatherIcon.frame = CGRect(x: (weatherView.frame.width - 130) / 2, y: 40, width: 130, height: 130)
            ...
            titleLabel.tag = 201
            titleLabel.frame = CGRect(x: 10, y: 180, width: weatherView.frame.width - 20, height: 24)
            ...
            descriptionLabel.frame = CGRect(x: 10, y: 208, width: weatherView.frame.width - 20, height: 20)
            ...
            locationLabel.frame = CGRect(x: 20, y: 238, width: weatherView.frame.width - 40, height: 32)
            ...
            DispatchQueue.global().async {
                if let data = try? Data(contentsOf: iconUrl), let image = UIImage(data: data) {
                    DispatchQueue.main.async {
                        ...
                        imageView.tintColor = nil // Remove tint for actual weather icons
                    }
                }
            }
        }
