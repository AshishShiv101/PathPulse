...
class CitySearchHelper {
    ...
    private func displayRouteInfoView(for route: MKRoute, at index: Int, mapView: MKMapView, isShortest: Bool, isLongest: Bool) {
        ...
        if isShortest {
            routeInfoView.layer.borderColor = UIColor.blue.cgColor
        } else if isLongest {
            routeInfoView.layer.borderColor = UIColor(red: 0.3, green: 0.6, blue: 0.9, alpha: 1.0).cgColor
        } else {
            routeInfoView.layer.borderColor = UIColor(red: 0.3, green: 0.6, blue: 0.9, alpha: 1.0).cgColor
        }
        ...
        closeButton.addAction(UIAction { _ in
            // Remove the overlay associated with this view
            if let overlayIndex = mapView.overlays.firstIndex(where: { ($0 as? MKPolyline)?.userInfo as? [String: Any] != nil && (($0 as? MKPolyline)?.userInfo as? [String: Any])?["index"] as? Int == index }) {
                mapView.removeOverlay(mapView.overlays[overlayIndex])
            }
            
            // Remove the view from superview and routeInfoViews array
            ...
            // Reassign positions to remaining views
