        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                guard let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
                      ...
                      let visibility = json["visibility"] as? Int else {
        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                ...
                let hourlyWeatherData = hourlyList.prefix(8).compactMap { hour -> WeatherData? in
                    ...
                    let visibility = hour["visibility"] as? Int ?? 10000 // Default if missing
                    return WeatherData(
                        temperature: temp,
                        humidity: humidity,
                        windSpeed: windSpeed,
...
struct WeatherData {
    ...
    var uvIndex: Double? // Optional, requires One Call API
}
...
struct DailyWeatherData {
    ...
    var pressure: Int // hPa
    var visibility: Int // meters
    var uvIndex: Double? // Optional
}

struct AirQuality {
    let aqi: Int // Air Quality Index (1-5: Good to Hazardous)
    let pm25: Double // Particulate Matter 2.5 (µg/m³)
