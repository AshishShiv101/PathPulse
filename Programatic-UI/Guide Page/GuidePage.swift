...
class GuidePage: UIViewController, UISearchBarDelegate, UICollectionViewDelegate, UICollectionViewDataSource, CLLocationManagerDelegate {
    ...
    private func setupCarousels() {
        ...
        let carouselTitles = ["Clinics", "Hospitals", "Hotels", "Pharmacies", "Gas Stations"] // Added "Gas Stations"
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        // Update only if the user has moved more than 1 km or if this is the first location update
    ...
    // MARK: - Fetching Nearby Places
    private func fetchNearbyPlaces() {
        guard let location = currentLocation else { return }
        let locationString = "\(location.latitude),\(location.longitude)"
        
        let categories = ["clinic", "hospital", "hotel", "pharmacy", "gas_station"] // Added "gas_station"
        let titles = ["Clinics", "Hospitals", "Hotels", "Pharmacies", "Gas Stations"] // Added "Gas Stations"
