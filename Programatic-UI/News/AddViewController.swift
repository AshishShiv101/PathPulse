...
class NewsViewController: UIViewController, CLLocationManagerDelegate, UIScrollViewDelegate {
    ...
    public var searchedCity: String? {
        didSet {
            updateCityLabel()
            if searchedCity != nil {
                fetchNewsForSearchedCity() // Fetch news immediately when searchedCity is set
            }
        }
    }
    ...
    override func viewDidLoad() {
        ...
        // Only fetch news for searchedCity if it's set, otherwise use location
        if let searchedCity = searchedCity {
            fetchNewsForSearchedCity()
        } else {
            // If no searchedCity, we'll let locationManager fetch current location news
        }
    }
    
    private func fetchNewsForSearchedCity() {
        guard let searchedCity = searchedCity else { return }
        let initialQuery = "\(searchedCity)-road-accident OR \(searchedCity)-weather OR \(searchedCity)-news"
        self.currentQuery = initialQuery
        fetchNewsData(query: initialQuery) { [weak self] success in
    ...
    private func setupLocationManager() {
        ...
        // Only start updating location if no searchedCity is provided
        if searchedCity == nil {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        // Only proceed if no searchedCity is set
    ...
    @objc private func buttonPressed(sender: UIButton) {
        ...
        if let buttonIndex = buttonStackView.arrangedSubviews.firstIndex(of: sender) {
            // Use searchedCity if available, otherwise fall back to currentLocation
