...
class NewsViewController: UIViewController, CLLocationManagerDelegate {
    ...
    private var locationButtonsStackView: UIStackView!
    ...
    private func setupView() {
        ...
        currentLocationButton.titleLabel?.font = .systemFont(ofSize: 14, weight: .medium)
        currentLocationButton.setTitleColor(.white, for: .normal)
        currentLocationButton.backgroundColor = UIColor(hex: "#40cbd8") // Highlighted color for selected state
        currentLocationButton.layer.cornerRadius = 5
        currentLocationButton.clipsToBounds = true
        currentLocationButton.addTarget(self, action: #selector(locationButtonPressed(_:)), for: .touchUpInside)
        currentLocationButton.tag = 0 // Tag to identify the button
        ...
        destinationButton.titleLabel?.font = .systemFont(ofSize: 14, weight: .medium)
        destinationButton.setTitleColor(.white, for: .normal)
        destinationButton.backgroundColor = .darkGray // Default color for unselected state
        destinationButton.layer.cornerRadius = 5
        destinationButton.clipsToBounds = true
        destinationButton.addTarget(self, action: #selector(locationButtonPressed(_:)), for: .touchUpInside)
        destinationButton.tag = 1 // Tag to identify the button
        
        // Create a horizontal stack view for the buttons
        let locationButtonsStackView = UIStackView(arrangedSubviews: [currentLocationButton, destinationButton])
        locationButtonsStackView.translatesAutoresizingMaskIntoConstraints = false
        locationButtonsStackView.axis = .horizontal
        locationButtonsStackView.spacing = 12
        locationButtonsStackView.alignment = .center
        locationButtonsStackView.distribution = .fillEqually
        view.addSubview(locationButtonsStackView)
        ...
        NSLayoutConstraint.activate([
            ...
            locationButtonsStackView.topAnchor.constraint(equalTo: titleLabel.bottomAnchor, constant: 20),
            locationButtonsStackView.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 20),
            locationButtonsStackView.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -20),
            locationButtonsStackView.heightAnchor.constraint(equalToConstant: 40),
            
            buttonStackView.topAnchor.constraint(equalTo: locationButtonsStackView.bottomAnchor, constant: 20),
            ...
            containerView.topAnchor.constraint(equalTo: buttonStackView.bottomAnchor, constant: 20),
                    containerView.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 20),
                    containerView.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -20),
                    containerView.bottomAnchor.constraint(equalTo: view.bottomAnchor)
        ])
        showCurrentLocationNews()

    }
    
    private func showCurrentLocationNews() {
        // Remove all subviews from the container view
        containerView.subviews.forEach { $0.removeFromSuperview() }

        // Add the scroll view and stack view for current location news
        containerView.addSubview(scrollView)
        scrollView.addSubview(stackView)

        // Set constraints for the scroll view and stack view
        NSLayoutConstraint.activate([
    ...
    private func showDestinationNews() {
        // Remove all subviews from the container view
        containerView.subviews.forEach { $0.removeFromSuperview() }

        // Create and add the destination news view
        let destinationView = DestinationNewsView()
        destinationView.translatesAutoresizingMaskIntoConstraints = false
        containerView.addSubview(destinationView)

        // Set constraints for the destination news view
        NSLayoutConstraint.activate([
            destinationView.topAnchor.constraint(equalTo: containerView.topAnchor),
            destinationView.leadingAnchor.constraint(equalTo: containerView.leadingAnchor),
            destinationView.trailingAnchor.constraint(equalTo: containerView.trailingAnchor),
            destinationView.bottomAnchor.constraint(equalTo: containerView.bottomAnchor)
        ])
    }
    ...
    @objc private func locationButtonPressed(_ sender: UIButton) {
        ...
        let buttons = [
            buttonStackView.arrangedSubviews[0] as? UIButton,
            buttonStackView.arrangedSubviews[1] as? UIButton
        ].compactMap { $0 }
        ...
        // Switch views based on the button pressed
        if sender.tag == 1 { // Destination button
            showDestinationNews()
        } else { // Current location button
            showCurrentLocationNews()
        }
    }
